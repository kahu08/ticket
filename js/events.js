...
var vm = new Vue({
  ...
  methods: {
    ...
    checkout: function() {
      ...
      axios.post('http://localhost:8086/api/frontend/checkout', {
          cart: cart,
    ...
    removeMerchandise: function(merchandise) {
      store.commit('REMOVE_CART_MERCHANDISE', merchandise)
    },
    ...
    getMerchandises: function() {
      ...
      axios.get('http://localhost:8086/api/frontend/merchandises')
        .then(function(response) {
          store.commit('UPDATE_MERCHANDISE_LIST', response.data.data)
        })
        .catch(function(error) {
          console.log(error)
        })
    ...
    getEventTotal: function(event) {
      ...
      for (var merchandise of event.merchandises) {
        if (merchandise.selected_quantity > 0) {
          total += (merchandise.selected_quantity * merchandise.amount);
        }
      }
      return total
    },
    ...
    isOptionSoldOut: function(event, option_id, option) {
      var index = 0;
      if (option === 'Seasonal') {
        index = 1;
      }
  ...
  computed: {
    ...
    cartMerchandises() {
      return store.state.cart.merchandises
    },
    cartCount() {
